...
import { getDriveReadToken, listDrivePhotos, importDrivePhoto } from '../utils/drivePhotoPicker'

export default function DrivePhotoPicker({ notebookId, onImported, onClose }) {
  ...
  const loadPage = async (t, pageToken) => {
    try {
      const { items, next } = await listDrivePhotos(t, pageToken)
  ...
  const handlePick = async (file) => {
    ...
    try {
      const photo = await importDrivePhoto(file, token, notebookId)
      onImported(photo.id)
      onClose()
    } catch (err) {
      setError(err.message)
    ...
    >
      ...
      >
        <div className="p-4 border-b border-stone-100 flex items-center justify-between flex-shrink-0">
          <div>
            <h2 className="font-semibold text-stone-900">Google Drive Photos</h2>
            <p className="text-xs text-stone-400 mt-0.5">
              {status === 'loaded'
                ? 'Click a photo to import it into your journal'
                : 'Loading your photos…'}
            </p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-3 min-h-0">
          ...
          {status === 'error' && (
            <div className="text-center py-12 space-y-3 px-6">
              <p className="text-sm text-red-500 font-medium">Could not connect to Google Drive</p>
              <p className="text-xs text-stone-500 leading-relaxed">{error}</p>
              <div className="bg-stone-50 rounded-lg p-4 text-left space-y-2">
                ...
                <ol className="text-xs text-stone-500 space-y-1 list-decimal pl-4">
                  ...
                  <li>APIs &amp; Services → OAuth consent screen → Scopes</li>
                  <li>Add scope: <code className="bg-stone-100 px-1 rounded">https://www.googleapis.com/auth/drive.readonly</code></li>
                </ol>
              </div>
            </div>
          )}
          ...
          {status === 'loaded' && photos.length === 0 && (
            <div className="text-center py-16 text-stone-400 text-sm">
              No photos found in your Google Drive.
            </div>
          )}
              ...
              <div className="grid grid-cols-4 sm:grid-cols-5 gap-1.5">
                {photos.map(file => (
                  ...
                  >
                    {file.thumbnailLink ? (
                      <img
                        src={file.thumbnailLink.replace(/=s\d+$/, '=s300')}
                        alt={file.name}
